...
/* TEST CASE : Forward only the datapoint(s) with an absolute change of 100 */
...
/* TEST CASE : Forward only the datapoint(s) with atleast 10% change */
...
/* TEST CASE : Forward only the datapoint(s) with an absolute change of 100 */
TEST(DELTA, AbsoluteNegativeValuesSendOnlyChangedDatapoints)
{
    PLUGIN_INFORMATION *info = plugin_info();
    ConfigCategory *config = new ConfigCategory("scale", info->config);
    ASSERT_NE(config, (ConfigCategory *)NULL);
...
/* TEST CASE : Forward only the datapoint(s) with atleast 10% change */
TEST(DELTA, PercentNegativeValuesSendOnlyChangedDatapoints)
{
    PLUGIN_INFORMATION *info = plugin_info();
    ConfigCategory *config = new ConfigCategory("scale", info->config);
    ASSERT_NE(config, (ConfigCategory *)NULL);
